...
class HGGEP(pl.LightningModule):
    def __init__(self, learning_rate=1e-5, fig_size=112, label=None,
        ...
        self.ViT2 = nn.Sequential(*[attn_block(dim, 16, 64, dim, dropout) for i in range(4)])
        self.ViT3 = nn.Sequential(*[attn_block(dim, 16, 64, dim, dropout) for i in range(4)])

        self.jknet = nn.Sequential(nn.LSTM(dim, dim, 4), SelectItem(0))

        self.hgnn = HypergraphNeuralNetwork(input_dim=dim, hidden_dim=dim//2, output_dim=dim)
